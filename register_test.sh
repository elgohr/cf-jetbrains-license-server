...
Called mock with: https://$HOSTNAME/register $JETBRAINS_USERNAME $JETBRAINS_PASSWORD $SERVER_NAME" ]
  then
...
Called mock with: https://$SERVER_USERNAME:$SERVER_PASSWORD@$HOSTNAME/register $JETBRAINS_USERNAME $JETBRAINS_PASSWORD $SERVER_NAME" ]
  then
    echo "Registration mock was not called as expected: $result"
    exit 1
  fi
...
function itErrorsWhenMultipleRoutesAreSetButNoSERVER_HOSTNAMEwasSpecified() {
  ...
  export SERVER_PASSWORD="SERVER_PASSWORD"
  result=$(exec ./register.sh)
  if [ "$result" != "Error: Server started with multiple routes ($HOSTNAME), but no SERVER_HOSTNAME was configured for registration" ]
  then
...
Called mock with: https://$SERVER_HOSTNAME/register $JETBRAINS_USERNAME $JETBRAINS_PASSWORD $SERVER_NAME" ]
  then
...
Trying to register https://$SERVER_HOSTNAME with $JETBRAINS_USERNAME as $SERVER_NAME
Using authentication
Called mock with: https://$SERVER_USERNAME:$SERVER_PASSWORD@$SERVER_HOSTNAME/register $JETBRAINS_USERNAME $JETBRAINS_PASSWORD $SERVER_NAME" ]
  then
